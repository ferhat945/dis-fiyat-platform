...
function isNewLead(
  ...
  const diff =
    now.getTime() - l.createdAt.getTime();
...
export default async function PanelDashboardPage(): Promise<JSX.Element> {
  ...
  /*
   * Klinik kredi ve Premium bilgileri
   */
  try {
    clinic =
    ...
    console.error(
      "PANEL_DASHBOARD_CLINIC_ERROR",
      e
    );
  }

  /*
   * Eski abonelik/kota bilgisi.
   *
   * Şimdilik geriye dönük bilgi göstermek
   * amacıyla korunuyor.
   */
  try {
    activeSub =
    ...
    console.error(
      "PANEL_DASHBOARD_SUBSCRIPTION_ERROR",
      e
    );
  }
  ...
  /*
   * Son 14 günlük satın alınmış lead
   * trendi.
   *
   * Eski unlocked:false assignment'lar
   * artık istatistiğe dahil edilmez.
   */
  const trendDays = 14;
  ...
  try {
    ...
    console.error(
      "PANEL_DASHBOARD_ASSIGNMENTS_ERROR",
      e
    );
  }

  /*
   * Son 24 saatte gerçekten açılmış /
   * satın alınmış lead sayısı.
   *
   * Marketplace'teki kilitli leadler
   * burada sayılmaz.
   */
  try {
    todayLeadCount =
    ...
    console.error(
      "PANEL_DASHBOARD_TODAY_COUNT_ERROR",
      e
    );
  }

  /*
   * Son satın alınan 10 lead.
   *
   * CRM status değeri artık ortak
   * Lead.status üzerinden değil,
   * kliniğe özel LeadAssignment.status
   * üzerinden okunur.
   */
  try {
    const assignments =
    ...
    console.error(
      "PANEL_DASHBOARD_LAST_LEADS_ERROR",
      e
    );
  }

  /*
   * Aktif şehir + hizmet kapsamı
   */
  try {
    coverageCount =
      await prisma.clinicCoverage.count({
        where: {
          clinicId:
            session.clinicId,
    ...
    console.error(
      "PANEL_DASHBOARD_COVERAGE_ERROR",
      e
    );
  }
    ...
    <div className="panelWrap">
      ...
      <div className="panelGrid3">
        <div className="panelStatCard panelStatCardAccent">
          <div className="panelStatTop">
            <div className="panelStatLabel">
              Kredi Bakiyesi
            </div>

            <div className="panelStatIcon">
              💎
            </div>
          </div>

          <div className="panelStatValue">
            {creditBalance}
          </div>
          ...
          <div className="panelStatFoot">
            <Link
              className="panelLink panelLinkStrong"
              href="/panel/abonelik"
            >
              Kredi Satın Al →
            </Link>
        ...
        <div className="panelStatCard">
          ...
          <div className="panelStatRow">
            ...
            <div className="panelStatBadge">
              {clinic?.autoRenewPremium
                ? "Oto yenileme açık"
                : "Standart"}
            </div>
          </div>
      ...
      {creditBalance <= 0 ? (
        <div className="panelQuotaAlert">
          <div>
            <div className="panelQuotaTitle">
              💎 Kredin yok
            </div>

            <div className="panelQuotaDesc">
              Yeni leadlerin iletişim
              bilgilerini açmak için
      ...
      <div className="panelCard">
        <div className="panelCardHead">
          <div>
            <div className="panelCardTitle">
              📊 Lead Trend (Son 14 Gün)
            </div>

            <div className="panelCardSub">
              Satın alınan leadlerin
              günlük dağılımı. Tepe
              gün:{" "}
              <strong>
                {maxV}
              </strong>
          ...
          <div className="panelCardHeadRight">
            ...
            <span className="panelPill panelPillSoft">
              Satın Alınan
            </span>
          </div>
        </div>
        ...
        <div className="panelChartShell">
          ...
          <div
            className="panelChartLabels"
            aria-hidden
          >
            {series.map((p) => (
              <div
                key={p.label}
                className="panelChartTick"
        ...
        <div className="panelChartLegend">
          <div className="panelLegendItem">
            <span className="panelLegendDot" />{" "}
            Günlük satın alınan lead
          </div>

          <div className="panelLegendItem panelLegendMuted">
            Not: Gerçek dönüşler klinik
            iletişimine göre değişir.
          </div>
        </div>
      </div>
      ...
      {hasActiveSub ? (
        <div className="panelAlert">
          <div className="panelAlertTitle">
            Eski abonelik bilgisi
          </div>

          <div className="panelAlertDesc">
            Eski kota sisteminde kalan:{" "}
            <strong>
              {remaining}
            </strong>{" "}
            / {quotaTotal} • Kullanım: %
            {quotaPct}. Yeni sistemde
            lead açma işlemleri kredi
            bakiyesi üzerinden ilerler.
          </div>
        </div>
      ) : null}
    </div>
